...
export async function handleGallery(
    ...
    if (method === "POST") {
      try {
        ...
        const maxOrder = (await env.DB.prepare(
          "SELECT COALESCE(MAX(sort_order),0)+1 as next FROM gallery_sections",
        ).first()) as any;
        await env.DB.prepare(
          "INSERT INTO gallery_sections (id, name, key, accepts_video, sort_order, created_at) VALUES (?,?,?,?,?,datetime('now'))",
        )
          .bind(
            id,
            name.trim(),
          ...
          .run();
        return json(
          await env.DB.prepare("SELECT * FROM gallery_sections WHERE id = ?")
            .bind(id)
            .first(),
          201,
        );
  ...
  if (pathname === "/api/admin/gallery/sections/reorder" && method === "POST") {
    try {
      ...
      const stmts = ids.map((id, idx) =>
        env.DB.prepare(
          "UPDATE gallery_sections SET sort_order = ? WHERE id = ?",
        ).bind(idx, id),
      );
      await env.DB.batch(stmts);
      return json({ ok: true });
    } catch {
      return jsonError("Failed to reorder sections", 500);
    }
  }

  if (
    pathname.match(/^\/api\/admin\/gallery\/sections\/([^/]+)\/cover$/) &&
    method === "PUT"
  ) {
    try {
      ...
      await env.DB.prepare(
        "UPDATE gallery_sections SET cover_media_id = ? WHERE id = ?",
      )
        .bind(media_id, sectionId)
        .run();
      return json({ ok: true });
    } catch {
      return jsonError("Failed to set cover", 500);
    }
  }

  if (
    pathname.startsWith("/api/admin/gallery/sections/") &&
    ...
    if (method === "PUT") {
      try {
        const { name, key, accepts_video } = ((await req.json()) as any) ?? {};
        await env.DB.prepare(
          "UPDATE gallery_sections SET name=COALESCE(?,name), key=COALESCE(?,key), accepts_video=COALESCE(?,accepts_video), updated_at=datetime('now') WHERE id=?",
        )
          .bind(
            name || null,
            key || null,
            accepts_video !== undefined ? (accepts_video ? 1 : 0) : null,
            sectionId,
          )
          .run();
        return json(
    ...
    if (method === "DELETE") {
      try {
        await env.DB.prepare(
          "UPDATE gallery_media SET deleted_at = datetime('now') WHERE section_id = ?",
        )
          .bind(sectionId)
          .run();
        await env.DB.prepare("DELETE FROM gallery_sections WHERE id = ?")
          .bind(sectionId)
          .run();
  ...
  if (pathname === "/api/admin/gallery/upload-url" && method === "POST") {
    try {
      const { filename, content_type, section_id } =
        ((await req.json()) as any) ?? {};
      if (!filename || !content_type || !section_id)
        return jsonError("filename, content_type, section_id required", 400);
  ...
  if (pathname === "/api/admin/gallery/media") {
    ...
    if (method === "POST") {
      try {
        ...
        if (!section_id || !key || !public_url)
          return jsonError("section_id, key, public_url required", 400);
        const id = crypto.randomUUID();
        const maxOrder = (await env.DB.prepare(
          "SELECT COALESCE(MAX(sort_order),0)+1 as next FROM gallery_media WHERE section_id=?",
        )
          .bind(section_id)
          .first()) as any;
        await env.DB.prepare(
          "INSERT INTO gallery_media (id, section_id, key, public_url, filename, content_type, size, media_type, sort_order, created_at) VALUES (?,?,?,?,?,?,?,?,?,datetime('now'))",
        )
          .bind(
            id,
            section_id,
          ...
          .run();
        return json(
          await env.DB.prepare("SELECT * FROM gallery_media WHERE id = ?")
            .bind(id)
            .first(),
          201,
        );
  ...
  if (pathname === "/api/admin/gallery/media/reorder" && method === "POST") {
    try {
      ...
      const stmts = ids.map((id, idx) =>
        env.DB.prepare(
          "UPDATE gallery_media SET sort_order = ? WHERE id = ?",
        ).bind(idx, id),
      );
      await env.DB.batch(stmts);
      return json({ ok: true });
    } catch {
      return jsonError("Failed to reorder media", 500);
    }
  }

  if (
    pathname.startsWith("/api/admin/gallery/media/") &&
    pathname !== "/api/admin/gallery/media/reorder"
  ) {
    ...
    if (method === "PUT") {
      try {
        ...
        await env.DB.prepare(
          "UPDATE gallery_media SET caption=?, updated_at=datetime('now') WHERE id=?",
        )
          .bind(caption || null, mediaId)
          .run();
        return json(
          await env.DB.prepare("SELECT * FROM gallery_media WHERE id = ?")
            .bind(mediaId)
    ...
    if (method === "DELETE") {
      try {
        const row = (await env.DB.prepare(
          "SELECT key FROM gallery_media WHERE id = ?",
        )
          .bind(mediaId)
          .first()) as any;
        if (!row) return jsonError("Media not found", 404);
        await env.MEDIA_BUCKET.delete(row.key);
        await env.DB.prepare("DELETE FROM gallery_media WHERE id=?")
          .bind(mediaId)
          .run();
        return new Response(null, { status: 204 });
      } catch {
